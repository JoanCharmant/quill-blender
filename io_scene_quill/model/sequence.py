...
# The schema can be found in quill-file-format/schema/sequence_schema.json
# The schema was also generated by Quicktype from sample data and manually edited.
# The generated code is manually post-processed to add default initializers.
#
# FIXME: the binding between layers and their implementation is less strict than
...
class LayerImplementation:
    def __init__(self, children, drawings, framerate, frames, max_repeat_count, allow_translation_x, allow_translation_y, allow_translation_z, color, exporting, showing_volume, sphere, type_str, version):
    ...
    def to_dict(self):
        ...
        if self.children is not None:
            result["Children"] = from_union([lambda x: from_list(lambda x: to_class(Layer, x), x), from_none], self.children)
...
class QuillSequence:
    ...
    def from_dict(obj):
        ...
        return QuillSequence(sequence, version)
    ...
    @staticmethod
    def from_default():
        sequence = Sequence.from_default()
        version = 1
        return QuillSequence(sequence, version)


def quill_sequence_from_dict(s):
    return QuillSequence.from_dict(s)


def quill_sequence_to_dict(x):
    return to_class(QuillSequence, x)


def quill_sequence_from_default():
    return QuillSequence.from_default()
